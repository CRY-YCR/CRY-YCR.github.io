...
$$ M^{(n)} = M^{(m)} \quad on\  \{ t\leq \tau_n\}.$$<br>
$$ \tilde{A}^{(n)} =\tilde{A}^{(m)} \quad on\  \{ t\leq \tau_n\}.  $$ <br>
...
$$M:=\sum_{n\geq 1} M^{(n)} \cdot 1_{E_n}. $$<br>
$$\tilde{A}:= \sum_{n \geq 1} \tilde{A}^{(n)}\cdot 1_{E_n}.  $$<br>
...
对于$\tilde{A}$来说, 一方面对于$a.s. \ \omega$和$T\gt0$, 存在$N\gt 0$, 使得$T \lt \tau_{N}(\omega)$, 故有:<br>  
$$|\tilde{A}|_t(\omega) \leq |\tilde{A}^{(n)}_t (\omega)| \lt \infty. $$<br>
因此$\tilde{A}$是一个有限变差过程.
